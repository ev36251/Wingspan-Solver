...
if _MODAL_AVAILABLE:
    ...
    @app.function(cpu=2, memory=4096, timeout=14400)
    def run_2p_tiering_shard_remote(task: dict) -> dict:
        """Play 2-player games for a shard of seeds; return per-player bird/score rows."""
        import os
        import tempfile
        os.environ.setdefault("OPENBLAS_NUM_THREADS", "1")
        os.environ.setdefault("MKL_NUM_THREADS", "1")
        ...
        from backend.ml.factorized_inference import FactorizedPolicyModel
        from backend.ml.state_encoder import StateEncoder
        from backend.ml.two_player_tiering import seed_rows
        ...
        try:
            ...
            rows = []
            for s in task["seeds"]:
                rows.extend(seed_rows(int(s), model, encoder, board, top_k))
        finally:
            os.unlink(tmp.name)
        return {"rows_gz": gzip.compress(json.dumps(rows).encode("utf-8"), 6)}


def dispatch_2p_tiering_modal(seeds, model_path, board, top_k=6, seeds_per_shard=4):
    """Fan 2-player tiering games across Modal containers; return all rows."""
    ...
    print(f"  [modal] dispatching {len(tasks)} tiering shards ({len(seeds)} games) …")
    all_rows: list[dict] = []
    with modal.enable_output(), app.run():
        for result in run_2p_tiering_shard_remote.map(tasks):
